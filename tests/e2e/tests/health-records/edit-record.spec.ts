...
async function registerAndLogin(page: import('@playwright/test').Page) {
  const ts = Date.now();
  const username = `edit_rec_${ts}`;
  const email = `edit_rec_${ts}@demo.com`;
...
test.describe('Health Records CRUD - 健康记录增删改查', () => {
  test('TC-3.5: Edit health record', async ({ page }) => {
    ...
    await page.locator('table tbody tr').first().getByRole('button', { name: /Edit|编辑/ }).click();
    ...
    await modal.getByRole('button', { name: /Update|更\s*新/ }).click();
    ...
    // Verify: Table shows updated blood pressure '125/85'
