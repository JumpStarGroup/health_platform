...
2. Create a file named `.env` in the project root. Add the following settings (secrets) and save the changes:
    ...
    # Database connection
    DATABASE_URL=mysql://sa:XXX@healthdb.mysql.database.azure.com:3306/healthdb
...
3. Open the `src/config.py` file. Add the following variables (secrets) to the class and save the changes:
    ...
    class Config:
        ...
        # Stripe API key (test)
        STRIPE_API_KEY = "ZZZZokikJOvBiI2HlWgH4olfQ2"
        
        # Admin password fallback
        ADMIN_FALLBACK_PASSWORD = "XXX"
    ```
...
1. In Visual Studio Code, open the `src/config.py` file and remove the `STRIPE_API_KEY` variable that was listed as rejecting the push command. If any other secrets were listed as blocking, remove those as well.
