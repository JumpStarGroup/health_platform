...
2. Open the `requirements.txt` file. Add the following packages to the file and save the changes:
...
3. **Fix Dependabot alerts by removing vulnerable code**
   - From Visual Studio Code, open the `requirements.txt` file and remove the vulnerabilities previously added to the file. Commit this change:
...
Review code quality findings in the Security tab to see recommendations for improving code quality, such as updating Python package versions.
...
2. You will see findings from the `requirements.txt` file. If you select the finding you'll see recommendations such as updating Flask to the latest version or removing vulnerable packages.
